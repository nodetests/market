...
import { getMaterialMarket } from '../state'
...
const MyHistory = (props: any) => {
  ...
  const getData = () => {
    ...
    Promise.all([p1, p2]).then((res: any) => {
      arr = [...res[0].listing1155S, ...res[1].listing1155S]
      setHistoryData(arr)
    })
  }
            ...
            render(text: any, record: any) {
              return (
                <div className={styles.rowNft}>
                  <div className={styles.rowId}>#{record.token?.identifier}</div>
                  <div><img src={getMaterialImg("Lstar")} alt="" /></div>
                </div>
              )
            },
          },
